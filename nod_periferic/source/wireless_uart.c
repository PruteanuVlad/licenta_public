...
#define ADC16_TEMP_SENSOR_CHANNEL 0U
#define ADC16_WATER_SENSOR_CHANNEL 0U
...
static uint16_t mCharMonitoredHandles[2] = { (uint16_t)value_temp, (uint16_t)value_light };
...
void BleApp_Init(void)
{
    ...
    init_erpc_host();
#endif
    //(void)BleApp_Start(gGapPeripheral_c);
//               BleApp_Start(gGapPeripheral_c);
}

static tmrTimerID_t dht22TimerId = gTmrInvalidTimerID_c;
static tmrTimerID_t advTimer = gTmrInvalidTimerID_c;
static tmrTimerID_t debounceTimer = gTmrInvalidTimerID_c;
DHT22_Result result_dht22;
uint8_t buffer[16];
float humidity=20.2;
float temp=10.5;
uint8_t door = 1;
...
int count = 0;
tmrErrCode_t result;
gpioInputPinConfig_t pDoorInputConfig;
...
uint8_t initial;
static void debounce(void *param){
	if(GpioReadPinInput(&pDoorInputConfig) == initial){
		FLib_MemSet(&gAppAdvertisingData.aAdStructures[2].aData[0], 0, 8);
		GpioReadPinInput(&pDoorInputConfig);
		FLib_MemCpy(&gAppAdvertisingData.aAdStructures[2].aData[0], buffer, 9);
		bleResult_t result;
		result = Gap_SetAdvertisingData(&gAppAdvertisingData, NULL);
		if(gBleSuccess_c != result)
		{
			panic(0, 0, 0, 0);
		}
		(void)App_StartAdvertising(BleApp_AdvertisingCallback, BleApp_ConnectionCallback);
		TMR_StartLowPowerTimer(debounceTimer, gTmrSingleShotTimer_c, 1000, Gap_StopAdvertising, NULL);
	}
}
void door_pin_isr(){
	initial = GpioReadPinInput(&pDoorInputConfig);
	GpioClearPinIntFlag(&pDoorInputConfig);
	TMR_StartLowPowerTimer(debounceTimer, gTmrSingleShotTimer_c, 500, (pfTmrCallBack_t)debounce, NULL);
}
#define ADC16_WATER_CHANNEL 2U
#define ADC16_LIGHT_CHANNEL 3U
static void tpm_isr(void* pParam)
{
	if(dht22_check_if_idle() == DHT22_OK)
		result_dht22 = dht22_init();
	if(result_dht22.error_code != 0)
	{

	}
	else{
		humidity = result_dht22.humidity/10.0;
		if(result_dht22.temperature>32768)
			temp = -(result_dht22.temperature - 32768);
		temp = result_dht22.temperature/10.0;
	}
    adc16ChannelConfigStruct.channelNumber = ADC16_WATER_CHANNEL;
    ADC16_SetChannelConfig(ADC0, 0U, &adc16ChannelConfigStruct);
	while (0U == (kADC16_ChannelConversionDoneFlag &
			ADC16_GetChannelStatusFlags(ADC0, 0U)))
    {
    }
   	water = ADC16_GetChannelConversionValue(ADC0, 0U);
   	water = water/65535*100;

   	adc16ChannelConfigStruct.channelNumber = ADC16_LIGHT_CHANNEL;
	ADC16_SetChannelConfig(ADC0, 0U, &adc16ChannelConfigStruct);
	while (0U == (kADC16_ChannelConversionDoneFlag &
			ADC16_GetChannelStatusFlags(ADC0, 0U)))
    {
    }
    light = ADC16_GetChannelConversionValue(ADC0, 0U);

	buffer[0]=(floor)(humidity);
	buffer[1]=(humidity-(floor)(humidity))*100;
	buffer[2]=(floor)(temp);
	buffer[3]=(temp-(floor)(temp))*100;
    buffer[4]=(floor)(light);
    buffer[5]=(light-(floor)(light))*100;
    buffer[6]=(floor)(water);
    buffer[7]=(water-(floor)(water))*100;
    buffer[8]=door;
	...
	count++;
	(void)App_StartAdvertising(BleApp_AdvertisingCallback, BleApp_ConnectionCallback);
	TMR_StartLowPowerTimer(advTimer, gTmrSingleShotTimer_c, 1000, Gap_StopAdvertising, NULL);
}
...
void BleApp_Start(gapRole_t gapRole)
{
	...
	dht22TimerId = TMR_AllocateTimer();
	advTimer = TMR_AllocateTimer();
	debounceTimer = TMR_AllocateTimer();
	tpm_config_t timer_config2={0};

	Door_Input();
	PTC3_InputInit();
	pDoorInputConfig.gpioPin = 4;
	pDoorInputConfig.gpioPort = gpioPort_C_c;
	pDoorInputConfig.interruptSelect = pinInt_EitherEdge_c;
	pDoorInputConfig.pullSelect = pinPull_Disabled_c;

	(void)GpioInstallIsr(door_pin_isr, 1, 1, &pDoorInputConfig);
	NVIC_EnableIRQ(PORTB_PORTC_IRQn);
	init_adc();
	PTB2_ADCInit();
	PTB3_ADCInit();
    ...
    GPIO_PinInit(GPIOA, 16U, &led_config);
	TMR_StartLowPowerTimer(dht22TimerId, gTmrIntervalTimer_c, 4000, tpm_isr, NULL);
...
static void BleApp_ServiceDiscoveryCallback(deviceId_t peerDeviceId, servDiscEvent_t *pEvent)
{
    switch (pEvent->eventType)
    {
        case gServiceDiscovered_c:
        {
            ...
            {
                if (FLib_MemCmp((void *)&uuid_service_sensor, (void *)&pEvent->eventData.pService->uuid, sizeof(bleUuid_t)))
                {
    ...
    gattServerEvent_t *pServerEvent)
{
    ...
    switch (pServerEvent->eventType)
    {
        ...
        {
            if (pServerEvent->eventData.attributeWrittenEvent.handle == (uint16_t)value_temp)
            {
                //BleApp_ReceivedUartStream(deviceId, pServerEvent->eventData.attributeWrittenEvent.aValue,
                //                          pServerEvent->eventData.attributeWrittenEvent.cValueLength);
                if(*(pServerEvent->eventData.attributeWrittenEvent.aValue) == 49)
                {
					GPIO_PortSet(GPIOA, 1u << 16U);
					GPIO_PortClear(GPIOC, 1u << 1U);
					OSA_TimeDelay(100);
					GPIO_PortClear(GPIOA, 1u << 16U);
                }
                if(*(pServerEvent->eventData.attributeWrittenEvent.aValue) == 48)
                {
					GPIO_PortSet(GPIOC, 1u << 1U);
					GPIO_PortClear(GPIOA, 1u << 16U);
					OSA_TimeDelay(100);
					GPIO_PortClear(GPIOC, 1u << 1U);
                }
            }
            if (pServerEvent->eventData.attributeWrittenEvent.handle == (uint16_t)value_light)
            {
                if(*(pServerEvent->eventData.attributeWrittenEvent.aValue) == 49)
                {
					GPIO_PortSet(GPIOA, 1u << 19U);
					GPIO_PortClear(GPIOC, 1u << 16U);
					OSA_TimeDelay(100);
...
void BleApp_StateMachineHandler(deviceId_t peerDeviceId, appEvent_t event)
{
    ...
    temp.pUuidArray = uuid_service_sensor;
